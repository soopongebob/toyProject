...
public interface ArticleRepository extends JpaRepository<Article, Long> {
    ...
    @Query(value = "SELECT * FROM ( " +
            "SELECT * FROM Article WHERE createddtm >= DATEADD('DAY', -7, CURRENT_DATE()) " +
            "UNION ALL " +
            "SELECT * FROM Article WHERE createddtm < DATEADD('DAY', -7, CURRENT_DATE()) " +
            ") t ORDER BY createddtm DESC LIMIT 5", nativeQuery = true)
    Page<Article> weeklyQuery(Pageable pageable);
